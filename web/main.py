...
def versions(request: Request, game: Dict[str, Any]):
    return render(request, 'html/game.html', {'game': game, 'versions': DB.execute('SELECT version, released FROM GameVersions WHERE game=? ORDER BY released DESC', (game['id'],)).fetchall()})


@route(r'/(\w+)/mods/', 'GET')
@require_game
def game_mods(request: Request, game: Dict[str, Any]):
    return render(request, 'html/game_mods.html', {'game': game, 'mods': DB.execute('SELECT Mods.* FROM Mods INNER JOIN ModVersions ON Mods.id=ModVersions.mod WHERE game_version IN (SELECT id FROM GameVersions WHERE game=?)', (game,)).fetchmany()})
...
def static(request: Request, file: str):
